...
public class CTestWindProtocel {
     ...
     */
    public static byte[] sendCmdV1 = { (byte) 0xA6, (byte) 0xA6, 0x02, 0x08, 0x54, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x19, 0x06 };
    public static byte[] sendCmdV2 = { (byte) 0xA6, (byte) 0xA6, 0x02, 0x02, 0x08, 0x54, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x19, 0x06 };
    /**
     * 第一种风压数据的协议头标志
     */
    public final static int FLAG_RECV_PROTOCOL_HEAD_1 = 0xC0;  //第二个0是基站号 可能不为0
    /**
     * 第二种风压数据的协议头标志
     */
    ...
    public static void createSendCmd(int modalChanelNo, int deviceNo, int line){
        if ("2.0".equals(SysParamsAll.get_protocolVer())){
            sendCmdV2[2] = (byte) modalChanelNo;
            sendCmdV2[3] = (byte) modalChanelNo;
            sendCmdV2[4] = (byte) deviceNo;
            sendCmdV2[5] = (byte) (line + 0x40);
            int sum = (int) getCheckSum(sendCmdV2, sendCmdV2.length, 2, sendCmdV2.length - 4);
            sendCmdV2[11] = (byte) (sum % 64);
            sendCmdV2[12] = (byte) (sum / 64);
        } else {
            sendCmdV1[2] = (byte) modalChanelNo;
            sendCmdV1[3] = (byte) deviceNo;
            sendCmdV1[4] = (byte) (line + 0x40);
            int sum = (int) getCheckSum(sendCmdV1, sendCmdV1.length, 2, sendCmdV1.length - 4);
            sendCmdV1[10] = (byte) (sum % 64);
            sendCmdV1[11] = (byte) (sum / 64);
        }
    }
    ...
    public static int splitPressureFromBuffer(byte[] buffer, int bufLen, int trackNo, PressureValue pd){
        ...
        //发现问题 如果头是C 那么需要与 0xC0
        if (FLAG_RECV_PROTOCOL_HEAD_1 == (protocolType & 0xC0)){
            ...
            //发现问题 如果头是B 那么需要与 0xF0
        } else if (FLAG_RECV_PROTOCOL_HEAD_2 == (protocolType & 0xF0)){
