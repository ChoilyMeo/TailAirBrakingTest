...
public class DataTransfer implements OnSerialPortDataListener, OnOpenSerialPortListener {
    ...
    public void startReplay(String testIndex){
        XLog.i("开始查询和回放记录：" + testIndex);
        if (!tData.loadData(testIndex)) {
            XLog.e("查询回放加载记录" + testIndex + "失败，对数据进行重置！");
            tData.reset();
        }
    }
    ...
    private synchronized void processOnPressureValue(PressureValue pd) {
        //压力小于50的判断只在试验开始的时候判断，因为货车的有紧急排风的情况，需要把压力降为0
        if (tData.lstPressureValue.size() <= SysParamsAll.get_maxTestPoint() && !pd.getPressureTime().isEmpty()
                && (pd.getPressureValue() >= MIN_ALLOWABLE_PRESSURE_VALUE || tData.lstPressureValue.size() > 0)) {
    ...
    public void onDataReceived(byte[] bytes) {
        try {
            ...
            XLog.i("接收到数据：" + HexUtil.bytesToHex(bytes));
            int protocolType;
            ...
            while (totalReceiveBuffer.size() >= CTestWindProtocel.RECV_PROTOCOL_DATA_LEN1 || totalReceiveBuffer.size() >= CTestWindProtocel.RECV_PROTOCOL_DATA_LEN2) {
                ...
                while (totalReceiveBuffer.size() > 0) {
                    ...
                    //2021-02-25发现关于帧头的判断有问题 需要分别与 0xC0和0xF0
                    if ((readByte & 0xC0) != CTestWindProtocel.FLAG_RECV_PROTOCOL_HEAD_1  && (readByte & 0xF0) != CTestWindProtocel.FLAG_RECV_PROTOCOL_HEAD_2 ) {
                ...
                if ((protocolType & 0xC0) == CTestWindProtocel.FLAG_RECV_PROTOCOL_HEAD_1 ) {
                ...
                } else if ((protocolType & 0xF0)== CTestWindProtocel.FLAG_RECV_PROTOCOL_HEAD_2 ) {
    ...
    public void onSuccess(File device) {
        XLog.i("打开串口" + device.getName() + ":" + SysParamsAll.get_baudRate() + "成功。");
    }
    ...
    public void onFail(File device, Status status) {
        XLog.i("打开串口" + device.getName() + ":" + SysParamsAll.get_baudRate() + "失败。");
    }
}
