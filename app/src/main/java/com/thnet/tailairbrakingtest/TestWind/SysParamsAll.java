...
public class SysParamsAll {
    ...
    private static String _portName = "COM4";
    ...
    private static byte _handDeviceNo = 0x00;
    ...
    private static int _noTail = 0;
    private static int _kehuoche = 1;//参数设置为0表示客车，1为货车
    private static int _hasComputer = 0;//有无计算机：0 有、1 无
    ...
    private static int _gdStandardTime = 60;
    private static int _adStandardTime = 60;
    private static int _adStandardLeak = 20;
    ...
    private static int _gdStandardDropSpeedMin = 10;
    private static int _gdStandardDropSpeedMax = 20;
    private static int _adStandardDropSpeedMin = 25;
    private static int _adStandardDropSpeedMax = 35;
    private static int _gdzdStandardDropTimeMin = 25;
    private static int _gdzdStandardDropTimeMax = 50;
    private static int _gdhjStandardDropTimeMin = 10;
    private static int _gdhjStandardDropTimeMax = 25;
    private static int _adStandardDropTimeMin = 49;
    private static int _adStandardDropTimeMax = 68;
    private static int _jnlxStandardLeak = 5;
    private static int _byStandardLeak = 5;
    ...
    private static int _wcKLWPressure = 30;
    ...
    public static List<SysParms> load(){
        ...
        for (SysParms parms : parmsList){
            ...
            if (parms.getParamID().endsWith("HandDeviceNo"))
            {
                _handDeviceNo = (byte)Integer.parseInt(parms.getParamValue());
            }
            ...
            if (parms.getParamID().endsWith("GDDropSpeedMin"))
            {
                _gdStandardDropSpeedMin = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("GDDropSpeedMax"))
            {
                _gdStandardDropSpeedMax = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("ADDropSpeedMin"))
            {
                _adStandardDropSpeedMin = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("ADDropSpeedMax"))
            {
                _adStandardDropSpeedMax = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("GDZDDropTimeMin"))
            {
                _gdzdStandardDropTimeMin = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("GDZDDropTimeMax"))
            {
                _gdzdStandardDropTimeMax = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("GDHJDropTimeMin"))
            {
                _gdhjStandardDropTimeMin = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("GDHJDropTimeMax"))
            {
                _gdhjStandardDropTimeMax = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("ADDropTimeMin"))
            {
                _adStandardDropTimeMin = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("ADDropTimeMax"))
            {
                _adStandardDropTimeMax = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("JNLXLeak"))
            {
                _jnlxStandardLeak = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("BYLeak"))
            {
                _byStandardLeak = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("NoTail"))
            {
                _noTail = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("kehuoche"))//客货车
            {
                _kehuoche = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("WCKLWPressure"))
            {
                _wcKLWPressure = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("JudgeEndTimes"))
            {
                _judgeEndTimes = Integer.parseInt(parms.getParamValue());
            }
            if (parms.getParamID().endsWith("HasComputer"))
            {
                _hasComputer = Integer.parseInt(parms.getParamValue());
            }
        }
        return parmsList;
    }
    ...
    public static byte get_handDeviceNo() {
        return _handDeviceNo;
    }

    public static void set_handDeviceNo(byte _handDeviceNo) {
        SysParamsAll._handDeviceNo = _handDeviceNo;
    }
    ...
    public static int get_gdStandardDropSpeedMin() {
        return _gdStandardDropSpeedMin;
    }

    public static void set_gdStandardDropSpeedMin(int _gdStandardDropSpeedMin) {
        SysParamsAll._gdStandardDropSpeedMin = _gdStandardDropSpeedMin;
    }

    public static int get_gdStandardDropSpeedMax() {
        return _gdStandardDropSpeedMax;
    }

    public static void set_gdStandardDropSpeedMax(int _gdStandardDropSpeedMax) {
        SysParamsAll._gdStandardDropSpeedMax = _gdStandardDropSpeedMax;
    }

    public static int get_adStandardDropSpeedMin() {
        return _adStandardDropSpeedMin;
    }

    public static void set_adStandardDropSpeedMin(int _adStandardDropSpeedMin) {
        SysParamsAll._adStandardDropSpeedMin = _adStandardDropSpeedMin;
    }

    public static int get_adStandardDropSpeedMax() {
        return _adStandardDropSpeedMax;
    }

    public static void set_adStandardDropSpeedMax(int _adStandardDropSpeedMax) {
        SysParamsAll._adStandardDropSpeedMax = _adStandardDropSpeedMax;
    }

    public static int get_gdzdStandardDropTimeMin() {
        return _gdzdStandardDropTimeMin;
    }

    public static void set_gdzdStandardDropTimeMin(int _gdzdStandardDropTimeMin) {
        SysParamsAll._gdzdStandardDropTimeMin = _gdzdStandardDropTimeMin;
    }

    public static int get_gdzdStandardDropTimeMax() {
        return _gdzdStandardDropTimeMax;
    }

    public static void set_gdzdStandardDropTimeMax(int _gdzdStandardDropTimeMax) {
        SysParamsAll._gdzdStandardDropTimeMax = _gdzdStandardDropTimeMax;
    }

    public static int get_gdhjStandardDropTimeMin() {
        return _gdhjStandardDropTimeMin;
    }

    public static void set_gdhjStandardDropTimeMin(int _gdhjStandardDropTimeMin) {
        SysParamsAll._gdhjStandardDropTimeMin = _gdhjStandardDropTimeMin;
    }

    public static int get_gdhjStandardDropTimeMax() {
        return _gdhjStandardDropTimeMax;
    }

    public static void set_gdhjStandardDropTimeMax(int _gdhjStandardDropTimeMax) {
        SysParamsAll._gdhjStandardDropTimeMax = _gdhjStandardDropTimeMax;
    }

    public static int get_adStandardDropTimeMin() {
        return _adStandardDropTimeMin;
    }

    public static void set_adStandardDropTimeMin(int _adStandardDropTimeMin) {
        SysParamsAll._adStandardDropTimeMin = _adStandardDropTimeMin;
    }

    public static int get_adStandardDropTimeMax() {
        return _adStandardDropTimeMax;
    }

    public static void set_adStandardDropTimeMax(int _adStandardDropTimeMax) {
        SysParamsAll._adStandardDropTimeMax = _adStandardDropTimeMax;
    }

    public static int get_jnlxStandardLeak() {
        return _jnlxStandardLeak;
    }

    public static void set_jnlxStandardLeak(int _jnlxStandardLeak) {
        SysParamsAll._jnlxStandardLeak = _jnlxStandardLeak;
    }
    ...
    public static int get_noTail() {
        return _noTail;
    }

    public static void set_noTail(int _noTail) {
        SysParamsAll._noTail = _noTail;
    }

    public static int get_kehuoche() {
        return _kehuoche;
    }

    public static void set_kehuoche(int _kehuoche) {
        SysParamsAll._kehuoche = _kehuoche;
    }

    public static int get_wcKLWPressure() {
        return _wcKLWPressure;
    }

    public static void set_wcKLWPressure(int _wcKLWPressure) {
        SysParamsAll._wcKLWPressure = _wcKLWPressure;
    }

    public static int get_hasComputer() {
        return _hasComputer;
    }

    public static void set_hasComputer(int _hasComputer) {
        SysParamsAll._hasComputer = _hasComputer;
    }
}
