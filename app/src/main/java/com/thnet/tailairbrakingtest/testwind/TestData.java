...
public class TestData {
    ...
    //解决多线程并发带来的保存数据异常（java.util.ConcurrentModificationException）的问题
    //public List<PressureValue> lstPressureValue = new ArrayList<PressureValue>();
    public List<PressureValue> lstPressureValue = Collections.synchronizedList(new ArrayList<>());
    ...
    public boolean loadData(String testIndex){
        try {
            ...
            if (null != windContentList && windContentList.size() > 0){
                ...
                XLog.i("查询加载数据完成，成功" + windContentList.size() + "条，使用记录：" + mTestContent.getTestID());
                return true;
            } else {
                XLog.i(TAG + "查询试验数据为空");
            }
        ...
        try {
            ...
            daoSession.runInTx(new Runnable() {
                ...
                public void run() {
                    ...
                    daoSession.getPressureValueDao().insertInTx(lstPressureValue);
                }
            });
            daoSession.clear();
            return true;
        ...
        if (null == getTestIndex() || getTestIndex().isEmpty())
        {
            ...
            try{
                ...
                if (!daoSession.callInTx(new Callable<Boolean>() {
                    ...
                    daoSession.clear();
                    XLog.i("保存数据成功，编号：" + mTestContent.getTestID());
                    return true;
                }
            } catch (Exception ex){
                setTestIndex("");
                //XLog.e(TAG + "数据更新异常：" + ex.getMessage());
                //日志中显示ex.getMessage为空，因此使用
                if (ex == null || ex.getMessage() == null) {
                    StringWriter sw = new StringWriter();
                    PrintWriter pw = new PrintWriter(sw);
                    ex.printStackTrace(pw);
                    String stackTrace = sw.toString();
                    XLog.e("数据更新异常，调用堆栈：" + stackTrace);
                } else {
                    XLog.e(TAG + "数据更新异常，异常信息：" + ex.getMessage());
                }
            }
        }
        else
